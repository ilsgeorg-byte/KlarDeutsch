# Руководство по безопасности и санитизации ввода данных в KlarDeutsch API
...
Для защиты от чрезмерного использования API и атак типа "brute force" была реализована система ограничения частоты запросов с использованием `flask-limiter`:

- **Общее ограничение**: 100 запросов в час по умолчанию для всех эндпоинтов
- **Аутентификация**: 5 запросов в минуту для предотвращения атак подбора паролей
- **AI-эндпоинты**: 20 запросов в минуту для предотвращения чрезмерного использования
- **Аудио-эндпоинты**: 10 запросов в минуту для предотвражения чрезмерной загрузки файлов
- **Другие эндпоинты**: 100 запросов в час

Ограничения настраиваются в файле `api/index.py` с использованием декоратора `@limiter.limit`.
