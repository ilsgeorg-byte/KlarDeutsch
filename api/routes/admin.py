...
from ..db import get_db_cursor # <-- Исправлен импорт на get_db_cursor
# from .schemas import UserSchema # UserSchema импортируется, но не используется, можно убрать или оставить
...
def get_admin_users():
    ...
    """
    # db = None # Удалено, т.к. get_db_cursor управляет соединением
    try:
        # Используем контекстный менеджер для получения курсора и управления соединением
        with get_db_cursor() as cur: 
            ...
            cur.execute(query_users_sql, tuple(params_for_users))
            ...
            cur.execute(query_count_sql, tuple(params_for_count))
    ...
    # finally блок для закрытия соединения удален, так как get_db_cursor управляет этим
