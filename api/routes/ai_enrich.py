...
1. ПЕРЕВОД (ru): Дай точный перевод. Если у слова есть несколько важных значений, укажи их через запятую (напр. "замок (здание), затвор").
2. ТЕМА (topic): Определи наиболее подходящую тему (напр. "Еда", "Путешествия", "Работа", "Здоровье", "Общее").
3. ОМОНИМЫ: Проверь, нет ли у слова разных значений в зависимости от рода (напр. das Schild vs der Schild). Используй артикль/перевод как якорь.
4. СУЩЕСТВИТЕЛЬНЫЕ: Артикль (der/die/das) + Plural (напр. "die Tische"). Если мн.ч. нет, пиши "-".
5. ГЛАГОЛЫ: 4 формы через запятую: "Infinitiv, Präsens(3sg), Präteritum(3sg), Partizip II (с haben/sein)".
6. СИНОНИМЫ/АНТОНИМЫ: Приведи ровно по 3 наиболее употребимых синонима и антонима через запятую.
7. КОЛЛОКАЦИИ (collocations): Приведи 3-5 наиболее частотных устойчивых сочетаний или идиом с этим словом.
8. ПРИМЕРЫ (examples): Напиши минимум 3 коротких, естественных примера. 
   - Пример 1: простой контекст.
   - Пример 2: использование в устойчивом выражении.
   - Пример 3: более сложный контекст.
